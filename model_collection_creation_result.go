/*
Jellyfin API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 10.10.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package jellyfin

import (
	"encoding/json"
)

// checks if the CollectionCreationResult type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CollectionCreationResult{}

// CollectionCreationResult struct for CollectionCreationResult
type CollectionCreationResult struct {
	Id *string `json:"Id,omitempty"`
}

// NewCollectionCreationResult instantiates a new CollectionCreationResult object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionCreationResult() *CollectionCreationResult {
	this := CollectionCreationResult{}
	return &this
}

// NewCollectionCreationResultWithDefaults instantiates a new CollectionCreationResult object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionCreationResultWithDefaults() *CollectionCreationResult {
	this := CollectionCreationResult{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *CollectionCreationResult) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionCreationResult) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *CollectionCreationResult) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *CollectionCreationResult) SetId(v string) {
	o.Id = &v
}

func (o CollectionCreationResult) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CollectionCreationResult) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["Id"] = o.Id
	}
	return toSerialize, nil
}

type NullableCollectionCreationResult struct {
	value *CollectionCreationResult
	isSet bool
}

func (v NullableCollectionCreationResult) Get() *CollectionCreationResult {
	return v.value
}

func (v *NullableCollectionCreationResult) Set(val *CollectionCreationResult) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionCreationResult) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionCreationResult) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionCreationResult(val *CollectionCreationResult) *NullableCollectionCreationResult {
	return &NullableCollectionCreationResult{value: val, isSet: true}
}

func (v NullableCollectionCreationResult) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionCreationResult) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


