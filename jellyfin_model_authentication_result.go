/*
Jellyfin API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 10.10.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package jellyfin

import (
	"encoding/json"
)

// checks if the JellyfinAuthenticationResult type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &JellyfinAuthenticationResult{}

// JellyfinAuthenticationResult A class representing an authentication result.
type JellyfinAuthenticationResult struct {
	// Class UserDto.
	User NullableJellyfinUserDto `json:"User,omitempty"`
	// Session info DTO.
	SessionInfo NullableJellyfinSessionInfoDto `json:"SessionInfo,omitempty"`
	// Gets or sets the access token.
	AccessToken NullableString `json:"AccessToken,omitempty"`
	// Gets or sets the server id.
	ServerId NullableString `json:"ServerId,omitempty"`
}

// NewJellyfinAuthenticationResult instantiates a new JellyfinAuthenticationResult object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJellyfinAuthenticationResult() *JellyfinAuthenticationResult {
	this := JellyfinAuthenticationResult{}
	return &this
}

// NewJellyfinAuthenticationResultWithDefaults instantiates a new JellyfinAuthenticationResult object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJellyfinAuthenticationResultWithDefaults() *JellyfinAuthenticationResult {
	this := JellyfinAuthenticationResult{}
	return &this
}

// GetUser returns the User field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JellyfinAuthenticationResult) GetUser() JellyfinUserDto {
	if o == nil || IsNil(o.User.Get()) {
		var ret JellyfinUserDto
		return ret
	}
	return *o.User.Get()
}

// GetUserOk returns a tuple with the User field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JellyfinAuthenticationResult) GetUserOk() (*JellyfinUserDto, bool) {
	if o == nil {
		return nil, false
	}
	return o.User.Get(), o.User.IsSet()
}

// HasUser returns a boolean if a field has been set.
func (o *JellyfinAuthenticationResult) HasUser() bool {
	if o != nil && o.User.IsSet() {
		return true
	}

	return false
}

// SetUser gets a reference to the given NullableJellyfinUserDto and assigns it to the User field.
func (o *JellyfinAuthenticationResult) SetUser(v JellyfinUserDto) {
	o.User.Set(&v)
}
// SetUserNil sets the value for User to be an explicit nil
func (o *JellyfinAuthenticationResult) SetUserNil() {
	o.User.Set(nil)
}

// UnsetUser ensures that no value is present for User, not even an explicit nil
func (o *JellyfinAuthenticationResult) UnsetUser() {
	o.User.Unset()
}

// GetSessionInfo returns the SessionInfo field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JellyfinAuthenticationResult) GetSessionInfo() JellyfinSessionInfoDto {
	if o == nil || IsNil(o.SessionInfo.Get()) {
		var ret JellyfinSessionInfoDto
		return ret
	}
	return *o.SessionInfo.Get()
}

// GetSessionInfoOk returns a tuple with the SessionInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JellyfinAuthenticationResult) GetSessionInfoOk() (*JellyfinSessionInfoDto, bool) {
	if o == nil {
		return nil, false
	}
	return o.SessionInfo.Get(), o.SessionInfo.IsSet()
}

// HasSessionInfo returns a boolean if a field has been set.
func (o *JellyfinAuthenticationResult) HasSessionInfo() bool {
	if o != nil && o.SessionInfo.IsSet() {
		return true
	}

	return false
}

// SetSessionInfo gets a reference to the given NullableJellyfinSessionInfoDto and assigns it to the SessionInfo field.
func (o *JellyfinAuthenticationResult) SetSessionInfo(v JellyfinSessionInfoDto) {
	o.SessionInfo.Set(&v)
}
// SetSessionInfoNil sets the value for SessionInfo to be an explicit nil
func (o *JellyfinAuthenticationResult) SetSessionInfoNil() {
	o.SessionInfo.Set(nil)
}

// UnsetSessionInfo ensures that no value is present for SessionInfo, not even an explicit nil
func (o *JellyfinAuthenticationResult) UnsetSessionInfo() {
	o.SessionInfo.Unset()
}

// GetAccessToken returns the AccessToken field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JellyfinAuthenticationResult) GetAccessToken() string {
	if o == nil || IsNil(o.AccessToken.Get()) {
		var ret string
		return ret
	}
	return *o.AccessToken.Get()
}

// GetAccessTokenOk returns a tuple with the AccessToken field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JellyfinAuthenticationResult) GetAccessTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AccessToken.Get(), o.AccessToken.IsSet()
}

// HasAccessToken returns a boolean if a field has been set.
func (o *JellyfinAuthenticationResult) HasAccessToken() bool {
	if o != nil && o.AccessToken.IsSet() {
		return true
	}

	return false
}

// SetAccessToken gets a reference to the given NullableString and assigns it to the AccessToken field.
func (o *JellyfinAuthenticationResult) SetAccessToken(v string) {
	o.AccessToken.Set(&v)
}
// SetAccessTokenNil sets the value for AccessToken to be an explicit nil
func (o *JellyfinAuthenticationResult) SetAccessTokenNil() {
	o.AccessToken.Set(nil)
}

// UnsetAccessToken ensures that no value is present for AccessToken, not even an explicit nil
func (o *JellyfinAuthenticationResult) UnsetAccessToken() {
	o.AccessToken.Unset()
}

// GetServerId returns the ServerId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JellyfinAuthenticationResult) GetServerId() string {
	if o == nil || IsNil(o.ServerId.Get()) {
		var ret string
		return ret
	}
	return *o.ServerId.Get()
}

// GetServerIdOk returns a tuple with the ServerId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JellyfinAuthenticationResult) GetServerIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ServerId.Get(), o.ServerId.IsSet()
}

// HasServerId returns a boolean if a field has been set.
func (o *JellyfinAuthenticationResult) HasServerId() bool {
	if o != nil && o.ServerId.IsSet() {
		return true
	}

	return false
}

// SetServerId gets a reference to the given NullableString and assigns it to the ServerId field.
func (o *JellyfinAuthenticationResult) SetServerId(v string) {
	o.ServerId.Set(&v)
}
// SetServerIdNil sets the value for ServerId to be an explicit nil
func (o *JellyfinAuthenticationResult) SetServerIdNil() {
	o.ServerId.Set(nil)
}

// UnsetServerId ensures that no value is present for ServerId, not even an explicit nil
func (o *JellyfinAuthenticationResult) UnsetServerId() {
	o.ServerId.Unset()
}

func (o JellyfinAuthenticationResult) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o JellyfinAuthenticationResult) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.User.IsSet() {
		toSerialize["User"] = o.User.Get()
	}
	if o.SessionInfo.IsSet() {
		toSerialize["SessionInfo"] = o.SessionInfo.Get()
	}
	if o.AccessToken.IsSet() {
		toSerialize["AccessToken"] = o.AccessToken.Get()
	}
	if o.ServerId.IsSet() {
		toSerialize["ServerId"] = o.ServerId.Get()
	}
	return toSerialize, nil
}

type NullableJellyfinAuthenticationResult struct {
	value *JellyfinAuthenticationResult
	isSet bool
}

func (v NullableJellyfinAuthenticationResult) Get() *JellyfinAuthenticationResult {
	return v.value
}

func (v *NullableJellyfinAuthenticationResult) Set(val *JellyfinAuthenticationResult) {
	v.value = val
	v.isSet = true
}

func (v NullableJellyfinAuthenticationResult) IsSet() bool {
	return v.isSet
}

func (v *NullableJellyfinAuthenticationResult) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJellyfinAuthenticationResult(val *JellyfinAuthenticationResult) *NullableJellyfinAuthenticationResult {
	return &NullableJellyfinAuthenticationResult{value: val, isSet: true}
}

func (v NullableJellyfinAuthenticationResult) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJellyfinAuthenticationResult) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


