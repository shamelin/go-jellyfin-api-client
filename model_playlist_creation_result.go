/*
Jellyfin API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 10.10.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the PlaylistCreationResult type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PlaylistCreationResult{}

// PlaylistCreationResult struct for PlaylistCreationResult
type PlaylistCreationResult struct {
	Id *string `json:"Id,omitempty"`
}

// NewPlaylistCreationResult instantiates a new PlaylistCreationResult object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPlaylistCreationResult() *PlaylistCreationResult {
	this := PlaylistCreationResult{}
	return &this
}

// NewPlaylistCreationResultWithDefaults instantiates a new PlaylistCreationResult object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPlaylistCreationResultWithDefaults() *PlaylistCreationResult {
	this := PlaylistCreationResult{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *PlaylistCreationResult) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PlaylistCreationResult) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *PlaylistCreationResult) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *PlaylistCreationResult) SetId(v string) {
	o.Id = &v
}

func (o PlaylistCreationResult) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PlaylistCreationResult) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["Id"] = o.Id
	}
	return toSerialize, nil
}

type NullablePlaylistCreationResult struct {
	value *PlaylistCreationResult
	isSet bool
}

func (v NullablePlaylistCreationResult) Get() *PlaylistCreationResult {
	return v.value
}

func (v *NullablePlaylistCreationResult) Set(val *PlaylistCreationResult) {
	v.value = val
	v.isSet = true
}

func (v NullablePlaylistCreationResult) IsSet() bool {
	return v.isSet
}

func (v *NullablePlaylistCreationResult) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePlaylistCreationResult(val *PlaylistCreationResult) *NullablePlaylistCreationResult {
	return &NullablePlaylistCreationResult{value: val, isSet: true}
}

func (v NullablePlaylistCreationResult) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePlaylistCreationResult) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


